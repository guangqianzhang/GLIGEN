...
class Trainer:
    def __init__(self, config):
        ...
        self.name, self.writer, checkpoint = create_expt_folder_with_auto_resuming(config.OUTPUT_ROOT,
        ...
        # 读取SD ckpt模型
        state_dict = read_official_ckpt(
            os.path.join(config.DATA_ROOT, config.official_ckpt_name))  # 'DATA/sd-v1-4.ckpt'
        ...
        dataset_train = ConCatDataset(config.train_dataset_names, config.DATA_ROOT, train=True,
                                      repeats=train_dataset_repeats)  # 合并数据集
        ...
        self.grounding_tokenizer_input = instantiate_from_config(config.grounding_tokenizer_input)  # data batch +mask
        self.model.grounding_tokenizer_input = self.grounding_tokenizer_input  # set in U-netmodel in openaimodel.py

        # func return input for grounding downsampler  
        self.grounding_downsampler_input = None
        if 'grounding_downsampler_input' in config:
            self.grounding_downsampler_input = instantiate_from_config(
                config.grounding_downsampler_input)  # batch['depth'],batch['sem']
    ...
    def get_input(self, batch):
        ...
        if self.grounding_downsampler_input != None:
            grounding_extra_input = self.grounding_downsampler_input.prepare( batch)  # return batch['depth'],batch['sem']
    ...
    def run_one_step(self, batch):
        ...
        grounding_input = self.grounding_tokenizer_input.prepare( batch)  # return {'depth': depth,"sem":sem, "mask":mask}
