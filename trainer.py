...
class Trainer:
    def __init__(self, config):
        ...
        additional_channels = self.model.additional_channel_from_downsampler  # temp channel
    ...
    def log_histogram(self, bolck, name, value, epoch):
        try:
            self.writer.add_histogram(tag=bolck + '/' + name.replace(bolck, '') + '_grad', values=value,
                                      global_step=epoch)
            self.writer.add_histogram(tag=bolck + '/' + name.replace(bolck, '') + '_data', values=value, global_step=epoch)
    ...
    def parallel_histogram_logging(self, global_step):
        ...
        for name, param in self.model.named_parameters():
            if bolck in name and 'bias' not in name:
                values = param.detach().cpu().numpy()
                thread = threading.Thread(target=self.log_histogram, args=(bolck,name, values, global_step))
                thread.start()
