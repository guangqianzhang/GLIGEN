...
if __name__ == "__main__":
    ...
    parser.add_argument("--yaml_file", type=str, default="configs/cc3m_depth.yaml", help="paths to base configs.")
    ...
    parser.add_argument("--ckpt", type=lambda x: x if type(x) == str and x.lower() != "none" else None, default=None,
                        help=("If given, then it will start training from this ckpt"
                              "It has higher prioty than official_ckpt_name, but lower than the ckpt found in "
                              "autoresuming (see trainer.py)"
    ...
    parser.add_argument('--randomize_fg_mask', default=False, type=lambda x: x.lower() == "true",
                        help="Only used if inpaint_mode is true. If true, 0.5 chance that fg mask will not be a box "
                             "but a random mask. See code for details")
    parser.add_argument('--random_add_bg_mask', default=False, type=lambda x: x.lower() == "true",
                        help="Only used if inpaint_mode is true. If true, 0.5 chance add arbitrary mask for the whole "
                             "image. See code for details")
    ...
    # --yaml_file=configs/ade_sem.yaml  --DATA_ROOT=../../DATA   --batch_size=4
